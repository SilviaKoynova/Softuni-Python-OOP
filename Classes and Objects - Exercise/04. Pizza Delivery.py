class PizzaDelivery:
    ...
    def make_order(self):
        self.ordered = True
        ingredients_items = ', '.join([f'{key}: {value}' for (key, value) in self.ingredients.items()])
        return f"You've ordered pizza {self.name} prepared with {ingredients_items} and the price will be {self.price}lv."
